...
class CSVLogger(Callback):
    ...
    def __init__(self, filename, separator=',', hash='', append=False):
        self.sep = separator

        self.filename = filename + hash
    ...
    def on_train_begin(self, logs=None):
        ...
        self.csv_file = io.open(self.filename,
                                ...
                                **self._open_args)
        print('saving logs in', self.filename)
...
class ReduceLROnPlateau(Callback):
    ...
    def __init__(self, monitor='val_loss', factor=0.1, patience=10,
        ...
        self.idx = kwargs.get('index', None)
        self._reset()
    ...
    def on_train_begin(self, logs=None):
        if self.idx is None:
            ...
            self.optimiser = self.params['optimiser'][self.idx]
...
class ModelCheckpoint(Callback):
    ...
    def __init__(self, filepath, monitor='val_loss', verbose=0, save_best_only=False, mode='auto', hash='', period=1):
        super(ModelCheckpoint, self).__init__()
        self.monitor = monitor
        self.verbose = verbose
        self.filepath = filepath + hash
        self.save_best_only = save_best_only
        self.period = period
        self.epochs_since_last_save = 0
        print('saving models in', self.filepath)
...
class EnsembleCheckpoint(Callback):
    """Save the model after every epoch.
    ...
    def __init__(self, filepath, monitor='val_loss', verbose=0, save_best_only=False, mode='auto', hash='', period=1):
        super(EnsembleCheckpoint, self).__init__()
        self.monitor = monitor
        self.verbose = verbose
        self.filepath = filepath + hash
        self.save_best_only = save_best_only
            ...
            if self.save_best_only:
                ...
                if current is None:
                    ...
                    if self.monitor_op(current, self.best):
                        ...
                        self.best = current
                        for i, model in enumerate(self.model):
                            torch.save(model.state_dict(), filepath + f"_{i}.pth")
                    else:
                        if self.verbose > 0:
                            print('\nEpoch %05d: %s did not improve from %0.5f' %
                                  (epoch + 1, self.monitor, self.best))
