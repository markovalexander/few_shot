...
callbacks = [
    ...
    EnsembleCheckpoint(
        filepath=PATH + f'/models/maml_ens/finetune_{logs_name}_{args.train_pred_mode}_{args.epochs}.pth',
        monitor=f'val_{args.n}-shot_{args.k}-way_acc',
        hash=hash
    ),
    ReduceLRCallback,
    CSVLogger(PATH + f'/logs/maml_ens/finetune_{logs_name}_{args.train_pred_mode}_{args.epochs}.csv',
              hash=hash),
]
